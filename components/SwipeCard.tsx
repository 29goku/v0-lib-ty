...
export default function SwipeCard({
  ...
  // refs for measuring card content
  ...
  const scale = useTransform(x, [-300, -200, 0, 200, 300], [0.98, 0.99, 1, 0.99, 0.98])

  // Visual indicator transforms with smoother transitions
  const prevIndicatorOpacity = useTransform(x, [-300, -80, 0], [1, 0.6, 0])
  const prevIndicatorScale = useTransform(x, [-300, -80, 0], [1.1, 1, 0.9])
  const nextIndicatorOpacity = useTransform(x, [0, 80, 300], [0, 0.6, 1])
  const nextIndicatorScale = useTransform(x, [0, 80, 300], [0.9, 1, 1.1])
  ...
  const handleDragEnd = async (event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
    ...
    // More conservative swipe detection for smoother experience
    const shouldSwipe = absVelocity >= 600 || absOffset >= 150
            ...
            {!showAnswer && (
              <div className="mt-8 text-center space-y-4">
                ...
                <div className="flex justify-center space-x-8 text-sm md:text-base">
                  <div className="text-blue-400 font-bold">← Swipe Left: Previous</div>
                  <div className="text-green-400 font-bold">Swipe Right: Next →</div>
                </div>
                <div className="text-xs text-gray-400 font-medium">
                  Drag the card horizontally or use keyboard arrows to navigate
                </div>
              </div>
            )}
